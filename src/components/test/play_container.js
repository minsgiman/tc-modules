...
class playContainer {
    constructor(param) {
        this.playEventCb = param.eventCb;
        this.shopId = param.shopId;

        this.timeline = null;
        this.player = null;
        this.fullscreenFlag = false;
        this.zoomZoneBottom = 149;
        this.isLive = true;
        this.currentCamera = {};
        this.config = {};
        ...
        this.timeRange = 60;
        this.nowScale = "1h";
        this.cachedTimelineparams = null;
    ...
    livePlayStart (mediaUrl) {
        if (this.currentCamera.recorderType == "nvr") {
            if (this.player) {
                this.player.setPath(getNvrServerUrl(this.currentCamera.mediaStreamURL), this.currentCamera.channel);
            }
        } else if (this.currentCamera.recorderType == "recorder") {
            //TODO:
            //$scope.webRTCStart(0);
        } else {
            toastcamAPIs.call(this.shareStatus ? toastcamAPIs.camera.GET_SHARE_CAM_TOKEN : toastcamAPIs.camera.GET_TOKEN, {cameraId: this.currentCamera.id}, (res) => {
                setTimeout(() => {
                    if (this.player) {
                        this.player.setPath(mediaUrl, this.currentCamera.id + '?token=' + res.token);
                    }
                }, 1000);
            }, (err) => {
                console.log(err);
    ...
    cameraConnectOff () {
        ...
        this.showCameraStreamLayer = false;
        this.showNextPlayLayer = false;
        toastcamAPIs.call(toastcamAPIs.camera.CHECK_IS_LAST_RECORD, {cameraId: this.currentCamera.id}, (data) => {
    ...
    onTimelineEvent (event, data) {
        switch (event) {
            case 'clickedCVRArea':
                this.clickedCVRArea(data);
                break;
            case 'goLive':
                this.goLive();
                break;
            case 'setupDomain':
                this.setupDomain(data);
                break;
            ...
            case 'cursorDragStart':
                ...
                break;
            case 'cursorDragging':
                this.playEventCb('cursorDragging', data);
                break;
            case 'cursorDragEnd':
            ...
            case 'newTimelineDragCntUpdate':
                this.newTimelineDragCnt = data;
                break;
            case 'cursorDragStatusChanged':
                this.cursorDragStatus = data;
                break;
            case 'changeTimeRangeFlagUpdate':
                this.playEventCb('changeTimeRangeFlagUpdate', data);
                break;
            case 'dragThumCancleUpdate':
                this.dragThumCancle = data;
                break;
            case 'liveReloadCntUpdate':
                this.liveReloadCnt = data;
                break;
            case 'timelineMapChanged':
                this.playEventCb('timelineMapChanged', data);
                break;
            case 'timelineSgidWidthMapChanged':
                this.playEventCb('timelineSgidWidthMapChanged', data);
                break;
            case 'timelineSgidWidthMapPut':
                this.playEventCb('timelineSgidWidthMapPut', data);
                break;
            default:
                break;
        }
    ...
    initPlayer () {
        this.player = new flashPlayer({
            ...
            varServerUrl: '',
            eventCb: this.flashEventCallback.bind(this)
        });
    ...
    initTimeline () {
        this.timeline = new timeline({
            ...
            width: $("#timeline_table").width(),
            height: $("#timebar_area").height(),
            timeRange: 60,
            currentCamera: this.currentCamera,
            eventCb: this.onTimelineEvent.bind(this)
        });
        ...
        setTimeout(() => {
            ...
            if(this.cvrCheck == true || this.goCvrStatus == true){
                ...
                toastcamAPIs.call(this.shareStatus ? toastcamAPIs.camera.GET_SHARE_CAM_TOKEN : toastcamAPIs.camera.GET_TOKEN, {cameraId: this.currentCamera.id}, (res) => {
                    if (this.currentCamera.recorderType == "nvr") {
                        if (this.player) {
                            this.player.setPath(getNvrServerUrl(this.currentCamera.mediaStreamURL), this.currentCamera.channel + "?time=" + time.getTime());
                        }
                        this.plyBtnStatus = true;
                    } else if (this.currentCamera.recorderType == "recorder") {
                        //TODO:
    ...
    zoomVideoWithZoomablePoint (x, y) {
    ...
    fullscreenChangeEvent () {
        ...
        if (this.isFullScreen) {
            ...
            if (isExpiredCloud(this.currentCamera)) {
                if (this.player) {
        ...
        if(this.cvrCheck == false){
            if(this.lineMoveFlag == false && this.plyBtnStatus == true){
                ...
                if(checkNextTime == 1){
                    ...
                    this.currentCamera.lastRecDateString = lastRecMoment.locale($("html").attr("lang")).format(videoDateFormat);
                    this.currentCamera.lastRecTimeString = lastRecMoment.locale($("html").attr("lang")).format(videoTimeFormat);
    ...
    jumpToNextRecord () {
        var callbackFunc = function() {
            if (this.currentCamera.recordType == "event") {
                ...
                toastcamAPIs.call(toastcamAPIs.camera.FIND_CVR, {cameraId: this.currentCamera.id, cvrId: findTime, findDirection: 'next'}, (cvrData) => {
